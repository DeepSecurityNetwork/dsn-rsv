...
pub fn disintegrate_btc_msgs_and_sigs(msg: &[u8], sig: &[u8], is_ecdsa: bool) -> Option<(Vec<Vec<u8>>, Vec<Vec<u8>>)> {
    let msgs = match deepsafe_types::extra::disintegrate_btc_msg(&hex::encode(msg)) {
        Ok(msg) => {
...
fn test_eth_abi_encode() {
    ...
    let (txs, hash_to_sign, values, is_brc20) = deepsafe_types::extra::disintegrate_btc_msg(msg).unwrap();
